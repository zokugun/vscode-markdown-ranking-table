...
export async function setRanking() {
	...
	// Prompt for new ranking
	const newRankingInput = await vscode.window.showInputBox({
		prompt: `Set new ranking for column "${headerCells[rankingCol].trim()}"`,
		validateInput(value) {
			if(value.trim() === '') {
				return;
			}

			return /^\d+$/.test(value) ? undefined : 'Please enter a number';
		},
	});
	if(newRankingInput === undefined) {
		return;
	}

	const newRanking = Number.parseInt(newRankingInput, 10);
	...
	if(setRanking) {
		...
		currentRanking.value = newRanking;
		...
		for(const row of rankings) {
			...
			if(row.value > currentRanking.value && row.value <= newRanking) {
				const cellValue = row.cells[rankingCol];
			...
			else if(row.value < currentRanking.value && row.value >= newRanking) {
				const cellValue = row.cells[rankingCol];
